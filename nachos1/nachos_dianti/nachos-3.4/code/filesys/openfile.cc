...
OpenFile::~OpenFile()
{
    synchDisk->RemoveNum(hdr->Sectors);
    delete hdr;
}
...
{
   synchDisk->AddReader(hdr->Sectors);
   int result = ReadAt(into, numBytes, seekPosition);
   seekPosition += result;
   time_t t;
   time(&t);
   hdr->visitTime=t;
   hdr->WriteBack(hdr->Sectors);
   synchDisk->RemoveReader(hdr->Sectors);
   return result;
}
...
{
   synchDisk->AddWriter(hdr->Sectors);
   ...
   hdr->modTime=t;
   hdr->WriteBack(hdr->Sectors);
   synchDisk->RemoveWriter(hdr->Sectors);
   return result;
}
...
OpenFile::WriteAt(char *from, int numBytes, int position)
{
	...
	bool flag=hdr->Extend(freeMap,position+numBytes-fileLength);
	hdr->WriteBack(hdr->Sectors);
...
//     printf('f', "Writing %d bytes at %d, from file of length %d.\n",                           numBytes, position, fileLength);
