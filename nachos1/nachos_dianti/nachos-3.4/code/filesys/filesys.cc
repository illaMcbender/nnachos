...
FileSystem::FileSystem(bool format)
{
 ...
 ASSERT(nameHdr->Allocate(freeMap, SectorSize));
...
//----------------------------------------------------------------------
int 
FileSystem::PipeRead(char *data)
{
	FileHeader *pipeHdr=new FileHeader;
	pipeHdr->FetchFrom(PipeSector);
	int len=pipeHdr->getSize();
...
void
FileSystem::PipeWrite(char *data,int len)
{
	OpenFile *pipefile=new OpenFile(PipeSector);
...
FileSystem::Create(char *name, int initialSize)
{
    ...
    int dirSector=directory->FindD(name);
            ...
            if(initialSize==-1)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if(flag)
                        {
                            for(int i=name_pos+1;i<strlen(name);i++)
                            {
                                hdr->type[j]=name[i];
                                j++;
                            }
...
FileSystem::Remove(char *name)
{
    ...
    directory = new Directory(NumDirEntries);
    directory->FetchFrom(directoryFile);
    dirSector=directory->FindD(name);
    directory=new Directory(NumDirEntries);
    if(dirSector>=0)
    ...
    if(directory->GetType(filename)==0)
    {
     ...
     OpenFile *currentF=new OpenFile(sector);
     currentD->FetchFrom(currentF);
     if(!currentD->Empty())
     {
     printf("remove error!file exists\n");
     return FALSE;
     }
     delete currentD;
     delete currentF;
