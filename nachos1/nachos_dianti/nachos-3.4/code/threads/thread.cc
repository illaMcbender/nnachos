...
Thread::Suspend ()
{
    ...
    {
            if(machine->pageTable[i].threadID==tid)
            {
                    if(machine->pageTable[i].dirty==true)
                    {
                            int old=machine->pageTable[i].virtualPage;
                            memcpy(&(space->myDisk[old*PageSize]),&(machine->mainMemory[i*PageSize]),PageSize);
                    }
                    machine->pageTable[i].valid=false;
                    printf("Thread:%d,PhysPage:%d,unused\n",tid,i);
            }
    }
    scheduler->ReadyToRun(this);
    nextThread = scheduler->FindNextToRun();
